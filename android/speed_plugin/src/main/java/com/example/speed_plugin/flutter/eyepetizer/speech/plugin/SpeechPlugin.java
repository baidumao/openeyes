package com.example.speed_plugin.flutter.eyepetizer.speech.plugin;
...
public class SpeechPlugin implements MethodChannel.MethodCallHandler {
    ...
    public static SpeechPlugin registerWith(FlutterEngine flutterEngine,Activity activity) {
        MethodChannel methodChannel = new MethodChannel(flutterEngine.getDartExecutor(), "speech_plugin");
        SpeechPlugin speechPlugin = new SpeechPlugin(activity);
        methodChannel.setMethodCallHandler(speechPlugin);
        return speechPlugin;
    }

    private SpeechPlugin(Activity registrar) {
        activity = registrar;
    }
